// Scoring utilities for competition submissions
...
/**
 * Validate submission against test data structure
 */
export const validateSubmission = (
  submissionData: ParsedCSV,
  testRowCount: number,
  idColumn: string,
  targetColumn: string
  ...
  if (!submissionData.headers.includes(idCol)) {
    errors.push(`Missing required column: ${idColumn}`);
  }
  if (!submissionData.headers.includes(targetCol)) {
    errors.push(`Missing required column: ${targetColumn}`);
  }

  // Check row count matches test data
  if (submissionData.rowCount !== testRowCount) {
    errors.push(`Row count mismatch: expected ${testRowCount}, got ${submissionData.rowCount}`);
  }

  // Check for extra columns (only id and prediction should be present)
  const allowedColumns = [idCol, targetCol];
  const extraColumns = submissionData.headers.filter(h => !allowedColumns.includes(h));
  if (extraColumns.length > 0) {
    errors.push(`Extra columns not allowed: ${extraColumns.join(', ')}`);
  }
...
export const calculateAccuracy = (
  ...
  for (const row of submission.data) {
    ...
    if (actual !== undefined) {
      matched++;
      // For classification, compare directly or rounded for numeric
  ...
  if (matched === 0) {
    return { score: 0, valid: false, error: "No matching IDs found between submission and ground truth" };
  }
 ...
 * Returns inverse for leaderboard (higher is better)
 */
export const calculateRMSE = (
  ...
  if (matched === 0) {
    return { score: 0, valid: false, error: "No valid numeric predictions found" };
  }

  const rmse = Math.sqrt(sumSquaredError / matched);
  // Inverse so higher is better for leaderboard
...
export const scoreSubmission = (
  submission: ParsedCSV,
  groundTruth: ParsedCSV,
  metric: "accuracy" | "rmse" | "mae" | "f1" | "auc",
  idColumn: string = "id",
  ...
  switch (metric) {
    ...
    case "auc":
      // AUC not implemented, fall back to accuracy
      return calculateAccuracy(submission, groundTruth, idColumn, targetColumn);
    default:
      return calculateAccuracy(submission, groundTruth, idColumn, targetColumn);
  }
